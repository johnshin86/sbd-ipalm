function [A0, X0, Y] = genconvdata_sbd_constantX(params_in)
...
simslices = 25:30;            % For STM kernels, choose the slices
...
if strcmp(kerneltype, 'random')
    ...
    assert(N<=numel(simslices), ['Number of slices chosen for ' ...
        'simulated data -- numel(simslices) -- must be at least N.']);
    
    A0 = zeros([p p N]);  win = simslicewin;
    
    load('data/Data_N_50_nDef_1_thop_-0.200.mat', 'LDoS');
    for i = 1:N
        tmp = LDoS(131+(-win:win), 131+(-win:win), randi([min(simslices) max(simslices)]));
...
X0 = double(rand([m m]) <= theta) .* dist([m m]);

Y = zeros([m m N]);
for i = 1:N
    Y(:,:,i) = Y(:,:,i) + cconvfft2(A0(:,:,i), X0(:,:));
end

if (N > 1) || (N > 1)
    A0 = squeeze(mat2cell(A0, p, p, ones(1,N)))';
    X0 = squeeze(mat2cell(X0, m, m));
